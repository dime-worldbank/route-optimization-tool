...
try:
    ...
    arcpy.CalculateField_management("{}\\roadsMergedProjectedSelected.dbf".format(path), "KM", "!Shape_Length!/1000", "PYTHON","")
    ...
    codeblock = """def speedSet(pavement):
        if pavement == 'Paved':
            return {}
        else:
            return {}""".format(par_averageSpeedPaved, par_averageSpeed)
    ...
    fields = arcpy.ListFields(roadsNetwork_shp)

    # Print the field names
    for field in fields:
        print(field.name)

    arcpy.management.CalculateField(roadsNetwork_shp, "timeTaken", "!KM! / !Speed4Rout!", "PYTHON", "")
 ...
 # ROUTES PER DISTRICT - Snap markets to intersections and endings - all intersections and markets should always be at least road check points - but not river crossings #
    ...
    arcpy.Snap_edit(mktsFor2ndRound, 
                    [[roadIntersectionsAndEndPoints, "VERTEX", "800 Meters"], 
                    [roadsNetworkDissolve_shp, "EDGE", "800 Meters"]])
...
# Assuming that START_Y and END_Y are field names in roadsNetworkParts_shp
    ...
    arcpy.Near_analysis("{}\\marketsCloseToRoadsPre".format(path), roadsNetworkDissolve_shp, "500 Meters", "", "NO_ANGLE", "GEODESIC")
    ...
    arcpy.Near_analysis("{}\\feirasCloseToRoadsPre".format(path), roadsNetworkDissolve_shp, "500 Meters", "", "NO_ANGLE", "GEODESIC")
