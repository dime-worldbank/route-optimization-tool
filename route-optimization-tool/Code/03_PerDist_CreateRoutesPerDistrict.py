   ...
   # Check if Inp_listforMerge is a list and convert it to a string if necessary
    ...
    #arcpy.management.Merge(listforMerge, "{}\\RoutesMerged_Dist{}_Base{}_Day{}_Stage{}".format(Inp_path,Inp_dist,Inp_BaseName,Inp_d,Inp_Stage))
        ...
        arcpy.conversion.FeaturesToGPX("{}\\RoutesMergedDissolved_Dist{}_Base{}_Day{}_Stage{}".format(Inp_path,Inp_dist, Inp_BaseName,Inp_d,Inp_Stage),
                        "OutputVRP\\{}\\Base{}{}\\Day{}\\Trk_Dist{}_B{}_D{}_St{}Pre.gpx".format(Inp_dist,nameAdd2, Inp_base,Inp_d,Inp_dist, Inp_BaseName,Inp_d,Inp_Stage), "FIRST_Name","Shape_Length")
        print("FeaturesToGPX D{} St{}".format(Inp_d,Inp_Stage))
    except arcpy.ExecuteError:
        print(arcpy.GetMessages())
        print("Failed to execute Merge." , listforMerge )
    orig="OutputVRP\\{}\\Base{}{}\\Day{}\\Trk_Dist{}_B{}_D{}_St{}Pre.gpx".format(Inp_dist, nameAdd2, Inp_base,Inp_d,Inp_dist, Inp_BaseName,Inp_d,Inp_Stage)
    txt='Temporary\\inter{}.txt'.format(Inp_dist)
    txt2='Temporary\\inter2{}.txt'.format(Inp_dist)
    tgt="OutputVRP\\{}\\Base{}{}\\Day{}\\Trk_Dist{}_B{}_D{}_St{}.gpx".format(Inp_dist, nameAdd2,Inp_base,Inp_d,Inp_dist, Inp_BaseName,Inp_d,Inp_Stage)
    ...
    arcpy.CopyFeatures_management("{}\\RoutesMergedDissolved_Dist{}_Base{}_Day{}_Stage{}".format(Inp_path,Inp_dist,Inp_BaseName,Inp_d,Inp_Stage),
                                  "OutputVRP\\{}\\Base{}{}\\Day{}\\final_Dist{}_Base{}_Day{}_Stage{}.shp".format(Inp_dist,nameAdd2,Inp_base,Inp_d,Inp_dist,Inp_BaseName,Inp_d,Inp_Stage), "", "0", "0", "0")
    arcpy.AddGeometryAttributes_management("OutputVRP\\{}\\Base{}{}\\Day{}\\final_Dist{}_Base{}_Day{}_Stage{}.shp".format(Inp_dist,nameAdd2,Inp_base,Inp_d,Inp_dist,Inp_BaseName,Inp_d,Inp_Stage), "LINE_START_MID_END", "", "", "")
    cursor = arcpy.da.SearchCursor("OutputVRP\\{}\\Base{}{}\\Day{}\\final_Dist{}_Base{}_Day{}_Stage{}.shp".format(Inp_dist,nameAdd2,Inp_base,Inp_d,Inp_dist,Inp_BaseName,Inp_d,Inp_Stage), "END_Y")
    EndLat = []
    for row in cursor:
        EndLat.append(row[0])
    #print('EndLat', EndLat)
    cursor = arcpy.da.SearchCursor("OutputVRP\\{}\\Base{}{}\\Day{}\\final_Dist{}_Base{}_Day{}_Stage{}.shp".format(Inp_dist,nameAdd2,Inp_base,Inp_d,Inp_dist,Inp_BaseName,Inp_d,Inp_Stage), "END_X")
    EndLon = []
    for row in cursor:
        EndLon.append(row[0])
    #print('EndLon', EndLon)
...
def PerDist_function(Routes,Orders, nameAdd1, nameAdd2,count):
   ...
   # Set local variables
        ...
        time_zone_for_time_fields = None  # Adjust this as needed
        line_shape = "ALONG_NETWORK"  # or "STRAIGHT_LINES", based on your requirement
        time_window_factor = None  # Adjust this as needed
        excess_transit_factor = None  # Adjust this as needed
        generate_directions_on_solve = True  # Set to True if directions are needed
        spatial_clustering = "NO_CLUSTER"  # Adjust this as needed
        ignore_invalid_locations = True  # Set to True to ignore invalid locations
        ...
        layer_object = result_object.getOutput(0)
        print("LAyers in the project:",layer_object)
        ...
        #Suporder_field_mappings["MaxViolationTime"].defaultValue = 0
      ...
      #  routes_field_mappings["MaxTotalDistance"].mappedFieldName = "MaxTotalDistance"
        ...
        print("Script Completed and layer saved Successfully")
    ...
    for layer in arcpy.na.GetNAClassNames(layer_object):
        print("Survey layer",layer)

    # Save the routes as a feature class
    ...
    for base in range(1, count + 1):
        ...
        for d in range(1, 27):
            ...
            if "Route{}{}_{}".format(nameAdd1,base, d) in RouteValues:
                ...
                listname = []
                print("Initializing/resetting listname")
                listOfCkpts=[]
                #listOfCrossings=[]
                ...
                try:
                    ...
                    print("listname is empty. Continuing with the next steps...")
                ...
                for seq in range(0,maxSeq+1): # loop over all orders - from - to
                    ...
                    # Set the environment
                    arcpy.env.workspace = pathGDB  # Replace with your geodatabase path
                    arcpy.env.overwriteOutput = True

                    # Create a new closest facility analysis layer
                    closest_fac_layer = arcpy.na.MakeClosestFacilityAnalysisLayer(
                        network_data_source=network,  # Your network dataset
                        travel_mode= "New Travel Mode",  # Adjust as needed
                        ...
                        #accumulate_attributes=["Kilometers"],  # Adjust as needed
                        generate_directions_on_solve=True,
                        ignore_invalid_locations=True
                    ).getOutput(0)

                    # Add incidents (start points)
                    ...
                    #for layer in arcpy.na.GetNAClassNames(closest_fac_layer):
                     #   print(layer)
                ...
                for stgCrsPre in listOfCkpts:
                    ...
                    try:
                        arcpy.AddGeometryAttributes_management("C:/Users/idels/Documents/github/route-optimization-tool/OutputVRP/{}/Base{}{}/Day{}/final_Dist{}_Base{}_Day{}_Stage{}.shp".format(dist,nameAdd2,base,d,dist,BaseName,d,stgCrs), "LINE_START_MID_END", "", "", "")
                    except Exception as e:
                        print("Error in AddGeometryAttributes: {}".format(e))

                    try:
                        arcpy.MakeXYEventLayer_management("OutputVRP/{}/Base{}{}/Day{}/final_Dist{}_Base{}_Day{}_Stage{}.shp".format(dist,nameAdd2,base,d,dist,BaseName,d,stgCrs), "END_X", "END_Y", "{}/end_Dist{}_Base{}_Day{}_Stage{}".format(pathGDB,dist,BaseName,d,stgCrs),"", "")
                    except Exception as e:
                        print("Error in MakeXYEventLayer: {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in Buffer_analysis (Max): {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in Buffer_analysis (Min): {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in Clip_analysis: {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in Erase_analysis: {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in CreateRandomPoints: {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in SpatialJoin_analysis: {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in Sort_management: {}".format(e))

                    try:
                        arcpy.conversion.FeaturesToGPX("{}/Dist{}_Base{}_Day{}_Stage{}_RandomPointsWithDistSort".format(pathGDB,dist,BaseName,d,stgCrs),
                                                    "C:/Users/idels/Documents/github/route-optimization-tool/OutputVRP/{}/Base{}{}/Day{}/Ckpts_Dist{}_B{}_D{}_St{}.gpx".format(dist,nameAdd2,base,d,dist,BaseName,d,stgCrs), None, None, None, None)
                    except Exception as e:
                        print("Error in FeaturesToGPX: {}".format(e))
                    ...
                    try:
                        ...
                        print("Error in CopyFeatures: {}".format(e))
                ...
                print("imput to merge",listforMergePerDay)
                arcpy.management.Merge(listforMergePerDay, "OutputVRP\\{}\\Base{}{}\\Day{}\\RoutesMerged_Dist{}_Base{}_Day{}.shp".format(dist,nameAdd2,base,d,dist, BaseName,d))
